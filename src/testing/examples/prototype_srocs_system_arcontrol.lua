...
function step()
	...
	elseif state == "lower_end_effector" then
		regulate_end_effector_position(0.0)
		if(robot.proximity[14] > 0.99) then
			state = "pick_up_block"
			robot.joints.lift_fixture_vertical_link.set_target(0.0)
   		robot.magnets.magnet_0.set_current(3)
	   	robot.magnets.magnet_1.set_current(3)
			robot.magnets.magnet_2.set_current(3)
			robot.magnets.magnet_3.set_current(3)
			timer = 20;
		end
