...
namespace argos {
   ...
   REGISTER_VISUALIZATION(CQTOpenGLRender,
                          ...
                          "videos in a fast way. This can be done from within the ARGoS window and when running\n"
                          "ARGoS without visualizations (headless). To do it from within the ARGoS window,\n"
                          "you just need to press the red capture button and frame grabbing will be on. \n"
                          "You can also grab frames when running without visualizations by running ARGoS under\n"
                          "Xvfb to give it a virtual framebuffer to render into. The command\n\n"
                          "xvfb-run -s \"-screen 0 1600x1200x24\" argos3 -c example.argos\n\n"
                          "will run spawn a new Xvfb server with a virtual window size of 1600x1200, 8-bit color\n"
                          "per channel and run ARGoS under it (ARGoS can also be made to attach to an existing\n"
                          "Xvfb server via the DISPLAY environment variable). Refer to the documentation on Xvfb\n"
                          "for the precise meaning of the arguments.\n"
                          "By default, the frames are named 'frame_NNNNN.png' and are stored in the current\n"
                          "directory, i.e. the directory where you run the 'argos' command. If you want to override\n"
                          "these defaults, you can add the optional 'frame_grabbing' section as follows:\n\n"
                          "  <visualization>\n"
                          "    <qt-opengl>\n"
                          ...
                          "The 'headless_frame_rate' attribute specifes the frame skip rate (i.e. grab every\n"
                          "nth frame). The default value is '1'.",
                          "Usable"
      );
