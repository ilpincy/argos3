...
#include <argos3/plugins/simulator/physics_engines/physx/physx_dist/include/PxBroadPhase.h>
...
Friction algorithm selection not mutable, and must be set in PxSceneDesc at scene creation.
...
up to four scalar solver constraints per pair of touching objects.  The patch friction model is the same basic strong friction algorithm as PhysX 3.2 and before, with a
significant change that a defect in the friction strength computation has been fixed.  This defect in PhysX 3.2 and prior has applied a friction force twice as strong
as it should have been in cases when the shapes involved shared two or more contacts.  Thus the involved materials' friction coefficients should be doubled if the same
behavior is needed when migrating an existing application.  Note however that this will still result in a change of friction force applied in the less common case of a 
single contact point, which is both more physically correct but also likely negligible.  If however the erroneous discrepancy between single and multiple contact 
behavior must be retained, it can be selected as the deprecated ::eLEGACY_PATCH selection.
...
*/
	struct PxFrictionType
	{
		enum Enum
		{
			...
			eLEGACY_PATCH		//!< Select patch friction model with behavior unchanged from PhysX version 3.2 and prior.
		};
	};
...
struct PxSceneFlag
{
	enum Enum
	{
		...
		*/
		...
		PxPairFlag::eCCD_LINEAR requires this flag to be specified.
		...
		@see PxRigidBodyFlag::eENABLE_CCD, PxPairFlag::eCCD_LINEAR, eDISABLE_CCD_RESWEEP
		*/
		eENABLE_CCD	=(1<<2),
		...
		@see PxRigidBodyFlag::eENABLE_CCD, PxPairFlag::eCCD_LINEAR, eENABLE_CCD
		*/
