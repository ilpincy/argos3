...
struct PxClothStretchConfig
{
	...
	*/
	PX_INLINE PxClothStretchConfig( PxReal stiffness=1.0f, 
		PxReal stiffnessMultiplier=1.0f, PxReal compressionLimit=1.0f, PxReal stretchLimit=1.0f) 
		: stiffness(stiffness)
...
struct PxClothTetherConfig
{
	...
	*/
	PX_INLINE PxClothTetherConfig(PxReal stiffness = 1.0f, PxReal stretchLimit = 1.0f) 
		: stiffness(stiffness), stretchLimit(stretchLimit) 
	{}
};
...
struct PxClothMotionConstraintConfig
{
	...
	*/
	...
	\brief Constructor sets to default.
	*/
	PX_INLINE PxClothMotionConstraintConfig(PxReal scale = 1.0f, PxReal bias = 0.0f, PxReal stiffness = 1.0f) 
		: scale(scale), bias(bias), stiffness(stiffness) 
	{}
};
...
public:
	...
	*/
	...
	\return PxClothParticleData pointer which provides access to positions and weight.
	\note This function does the same as lockParticleData(PxDataAccessFlag::eREADABLE).
	*/
