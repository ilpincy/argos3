...
struct PxClothCollisionSphere
{
	...
	*/
	...
	PxClothCollisionSphere(const PxVec3& pos, PxReal radius)
...
struct PxClothCollisionPlane
{
	...
	PxReal distance; //!< The distance from the origin (in the negative normal direction)

	/**
	\brief Default constructor, performs no initialization.
	*/
	PxClothCollisionPlane() {}
	PxClothCollisionPlane(const PxVec3& normal, PxReal distance)
...
struct PxClothCollisionTriangle
{
	...
	*/
	...
	PxClothCollisionTriangle(
		const PxVec3& vertex0, 
		const PxVec3& vertex1,
		const PxVec3& vertex2) :
		vertex0(vertex0),
		vertex1(vertex1),
		vertex2(vertex2) {}
};
