...
struct PxConvexFlag
{
	enum Enum
	{
		...
		*/
		...
		/**
		\brief Inflates the convex object according to skin width. If the convex hull compute fails, use this flag for more robust convex hull computation.
...
public:
	...
	*/
	...
	<p>It is possible to use triangles or directly polygons.</p>

	<p>Deprecated, we will be accepting directly polygons and polygon indices.</p>
	<p>To obtain polygons from your triangles use computeHullPolygons.</p>
	...
	*/
	...
	/**
	\brief Pointer to array of polygons.
	<p>Pointer to first polygon. Caller may add polygonStrideBytes bytes to the pointer to access the next polygon.</p>
	...
	*/
	...
	/**
	\brief Pointer to array of sorted polygon indices.
	<p>Pointer to first index. Caller may add indicesStrideBytes bytes to the pointer to access the next index.</p>
	...
	*/
	...
	Please note, that if vertex limit is used with inflation flag together, the inflation code might add more vertices than the limit
	because the algorithm tries to solve sharp edges problem. To ensure the exact vertex limit with inflate flag try 
	to use PxToolkit::createConvexMeshSafe()
...
PX_INLINE bool PxConvexMeshDesc::isValid() const
{
	...
	else
	{
		if(polygons.data)
		{
			if(polygons.count < 4) // we do require 2 neighbours for each vertex - 4 polygons at least
				return false;
