...
public:
	...
	*/
	...
	\param[in] contactDistance the distance from the limit at which it becomes active. Default is 0.01f scaled by the tolerance length scale

	@see PxJointLimitParameters PxTolerancesScale
	*/

	PxJointLinearLimit(const PxTolerancesScale& scale, PxReal extent, PxReal contactDistance = -1)
	: value(extent)
	{
		PxJointLimitParameters::contactDistance = contactDistance == -1 ? 0.01f*scale.length : contactDistance; 
	}
...
public:
	...
	*/
	...
	\param[in] contactDistance the distance from the limit at which it becomes active. Default is the lesser of 0.01f scaled by the tolerance length scale, and 0.49 * (upperLimit - lowerLimit)

	@see PxJointLimitParameters PxTolerancesScale
	*/

	PxJointLinearLimitPair(const PxTolerancesScale& scale, PxReal lowerLimit, PxReal upperLimit, PxReal contactDistance = -1)
	: upper(upperLimit)
	, lower(lowerLimit)
	{
		PxJointLimitParameters::contactDistance = contactDistance == -1 ? PxMin(scale.length * 0.01f, (upperLimit*0.49f-lowerLimit*0.49f)) : contactDistance; 
...
public:
	...
	*/
	...
	\param[in] contactDistance the distance from the limit at which it becomes active. Default is the lesser of 0.1 radians, and 0.49 * (upperLimit - lowerLimit)

	@see PxJointLimitParameters
	*/

	PxJointAngularLimitPair(PxReal lowerLimit, PxReal upperLimit, PxReal contactDistance = -1)
	: upper(upperLimit)
	, lower(lowerLimit)
	{
		PxJointLimitParameters::contactDistance = contactDistance ==-1 ? PxMin(0.1f, 0.49f*(upperLimit-lowerLimit)) : contactDistance;
...
public:
	...
	*/
	...
	\param[in] contactDistance the distance from the limit at which it becomes active. 	Default is the lesser of 0.1 radians, and 0.49 * the lower of the limit angles

	@see PxJointLimitParameters
	*/

	PxJointLimitCone(PxReal yLimitAngle, PxReal zLimitAngle, PxReal contactDistance = -1):
	  yAngle(yLimitAngle),
	  zAngle(zLimitAngle)
	  {
			PxJointLimitParameters::contactDistance = contactDistance == -1 ? PxMin(0.1f, PxMin(yLimitAngle, zLimitAngle)*0.49f) : contactDistance;
			bounceThreshold = 0.5f;
	  }
