/**
 * @file <argos3/plugins/robots/eye-bot/control_interface/ci_eyebot_proximity_sensor.cpp>
 *
...
namespace argos {
   ...
   CCI_EyeBotProximitySensor::CCI_EyeBotProximitySensor() :
      m_tReadings(24) {
   ...
   const CCI_EyeBotProximitySensor::TReadings& CCI_EyeBotProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_EyeBotProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_EyeBotProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_EyeBotProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
